      ...
      response.data.hits.hits.forEach((result) => {
        ...
        let content =
          result._source.content
            .substr(indexOfResult - 20, 230)
            .replace(new RegExp(searchValue, 'g'), function (x) {
        ...
        document.getElementById('results').innerHTML +=
          '<div class="result"><hr><h3>' +
          result._source.path.split('.')[0] +
          ' <span class="author">par ' +
          result._source.author +
          '</span> <span class="type">' +
          result._source.path.split('.')[1].toUpperCase() +
          '</span></h3><p>' +
          content +
          '</p><a href="#" class="btn">Voir le fichier</a></div>';
      });
    })
